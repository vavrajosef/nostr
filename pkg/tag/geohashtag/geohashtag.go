...
// New TBD
func New(eventID []byte, relayURL []byte) *GeohashTag {
	return &GeohashTag{
		Type:     tag.TypeEvent,
		EventID:  eventID,
		RelayURL: relayURL,
...
// GeohashTag is a tag for an event, including the event ID and relay URL.
...
// Encode encodes the GeohashTag into a byte slice.
func (t *GeohashTag) Encode() [][]byte {
	b := make([][]byte, 0)
...
func (t *GeohashTag) Marshal() []byte {
