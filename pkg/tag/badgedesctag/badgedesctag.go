...
// New TBD
func New(eventID []byte, relayURL []byte) *BadgeDescTag {
	return &BadgeDescTag{
		Type:     tag.TypeEvent,
		EventID:  eventID,
		RelayURL: relayURL,
...
// BadgeDescTag is a tag for an event, including the event ID and relay URL.
...
// Encode encodes the BadgeDescTag into a byte slice.
func (t *BadgeDescTag) Encode() [][]byte {
	b := make([][]byte, 0)
...
func (t *BadgeDescTag) Marshal() []byte {
