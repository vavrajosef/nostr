...
// New TBD
func New(eventID []byte, relayURL []byte) *ContentWarnTag {
	return &ContentWarnTag{
		Type:     tag.TypeEvent,
		EventID:  eventID,
		RelayURL: relayURL,
	}
...
// ContentWarnTag is a tag for an event, including the event ID and relay URL.
...
// Encode encodes the ContentWarnTag into a byte slice.
func (t *ContentWarnTag) Encode() [][]byte {
	b := make([][]byte, 0)
...
func (t *ContentWarnTag) Marshal() []byte {
