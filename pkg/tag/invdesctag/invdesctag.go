...
// New TBD
func New(eventID []byte, relayURL []byte) *InvDescTag {
	return &InvDescTag{
		Type:     tag.TypeEvent,
		EventID:  eventID,
		RelayURL: relayURL,
...
// InvDescTag is a tag for an event, including the event ID and relay URL.
...
// Encode encodes the InvDescTag into a byte slice.
func (t *InvDescTag) Encode() [][]byte {
	b := make([][]byte, 0)
...
func (t *InvDescTag) Marshal() []byte {
