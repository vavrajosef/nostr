...
// New TBD
func New(eventID []byte, relayURL []byte) *SummaryTag {
	return &SummaryTag{
		Type:     tag.TypeEvent,
		EventID:  eventID,
		RelayURL: relayURL,
...
// SummaryTag is a tag for an event, including the event ID and relay URL.
...
// Encode encodes the SummaryTag into a byte slice.
func (t *SummaryTag) Encode() [][]byte {
	b := make([][]byte, 0)
...
func (t *SummaryTag) Marshal() []byte {
