...
// New TBD
func New(eventID []byte, relayURL []byte) *PublishedAtTag {
	return &PublishedAtTag{
		Type:     tag.TypeEvent,
		EventID:  eventID,
		RelayURL: relayURL,
	}
...
// PublishedAtTag is a tag for an event, including the event ID and relay URL.
...
// Encode encodes the PublishedAtTag into a byte slice.
func (t *PublishedAtTag) Encode() [][]byte {
	b := make([][]byte, 0)
...
func (t *PublishedAtTag) Marshal() []byte {
