...
// New TBD
func New(eventID []byte, relayURL []byte) *RelayTag {
	return &RelayTag{
		Type:     tag.TypeEvent,
		EventID:  eventID,
		RelayURL: relayURL,
...
// RelayTag is a tag for an event, including the event ID and relay URL.
...
// Encode encodes the RelayTag into a byte slice.
func (t *RelayTag) Encode() [][]byte {
	b := make([][]byte, 0)
