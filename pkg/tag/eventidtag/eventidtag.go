...
// New TBD
func New(eventID []byte, relayURL []byte) *EventIDTag {
	return &EventIDTag{
		Type:     tag.TypeEvent,
		EventID:  eventID,
		RelayURL: relayURL,
...
// EventIDTag is a tag for an event, including the event ID and relay URL.
...
// Encode encodes the EventIDTag into a byte slice.
func (t *EventIDTag) Encode() [][]byte {
	b := make([][]byte, 0)
...
func (t *EventIDTag) Marshal() []byte {
