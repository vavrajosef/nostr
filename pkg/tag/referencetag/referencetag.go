...
// New TBD
func New(eventID []byte, relayURL []byte) *ReferenceTag {
	return &ReferenceTag{
		Type:     tag.TypeEvent,
		EventID:  eventID,
		RelayURL: relayURL,
...
// ReferenceTag is a tag for an event, including the event ID and relay URL.
...
// Encode encodes the ReferenceTag into a byte slice.
func (t *ReferenceTag) Encode() [][]byte {
	b := make([][]byte, 0)
...
func (t *ReferenceTag) Marshal() []byte {
