...
// New TBD
func New(eventID []byte, relayURL []byte) *TitleTag {
	return &TitleTag{
		Type:     tag.TypeEvent,
		EventID:  eventID,
		RelayURL: relayURL,
...
// TitleTag is a tag for an event, including the event ID and relay URL.
...
// Encode encodes the TitleTag into a byte slice.
func (t *TitleTag) Encode() [][]byte {
	b := make([][]byte, 0)
