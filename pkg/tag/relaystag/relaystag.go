...
// New TBD
func New(eventID []byte, relayURL []byte) *RelaysTag {
	return &RelaysTag{
		Type:     tag.TypeEvent,
		EventID:  eventID,
		RelayURL: relayURL,
...
// RelaysTag is a tag for an event, including the event ID and relay URL.
...
// Encode encodes the RelaysTag into a byte slice.
func (t *RelaysTag) Encode() [][]byte {
	b := make([][]byte, 0)
