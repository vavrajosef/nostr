...
// New TBD
func New(eventID []byte, relayURL []byte) *PreimageTag {
	return &PreimageTag{
		Type:     tag.TypeEvent,
		EventID:  eventID,
		RelayURL: relayURL,
...
// PreimageTag is a tag for an event, including the event ID and relay URL.
...
// Encode encodes the PreimageTag into a byte slice.
func (t *PreimageTag) Encode() [][]byte {
	b := make([][]byte, 0)
...
func (t *PreimageTag) Marshal() []byte {
