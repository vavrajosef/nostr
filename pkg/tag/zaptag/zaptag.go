...
// New TBD
func New(eventID []byte, relayURL []byte) *ZapTag {
	return &ZapTag{
		Type:     tag.TypeEvent,
		EventID:  eventID,
		RelayURL: relayURL,
...
// ZapTag is a tag for an event, including the event ID and relay URL.
type ZapTag struct {
...
// Encode encodes the ZapTag into a byte slice.
func (t *ZapTag) Encode() [][]byte {
	b := make([][]byte, 0)
