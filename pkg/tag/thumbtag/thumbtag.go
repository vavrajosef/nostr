...
// New TBD
func New(eventID []byte, relayURL []byte) *ThumbTag {
	return &ThumbTag{
		Type:     tag.TypeEvent,
		EventID:  eventID,
		RelayURL: relayURL,
...
// ThumbTag is a tag for an event, including the event ID and relay URL.
...
// Encode encodes the ThumbTag into a byte slice.
func (t *ThumbTag) Encode() [][]byte {
	b := make([][]byte, 0)
